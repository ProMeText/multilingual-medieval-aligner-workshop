...
## usage : python tok_apply.py model_name tokenizer_name folder num output_dir
## where :
...
# folder is the path to the folder where are stored the files we want to tokenize
...
if __name__ == '__main__':
    ...
    directory = os.fsencode(sys.argv[3])
    ...
    for filename in listdir(directory):
        ...
        if isfile(full_path):
            with open(full_path) as f:
                ...
                #final_filename = str(filename).split('.')[0]
                output_file = join(output_dir, f'{final_filename}-tok.txt')
                    #tokText = tokenizeText(text)
