...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:tei="http://www.tei-c.org/ns/1.0"
    ...
    <xsl:param name="punctuation_regex"/>
...
-->
    ...
    <xsl:template
        ...
        <xsl:for-each select="tokenize(., '\s+')">
            <xsl:analyze-string select="." regex="{$punctuation_regex}">
    ...
    <xsl:template match="/">
        ...
        <xsl:message select="$punctuation_regex"></xsl:message>
            <xsl:result-document href="{$output_path}">
                <xsl:apply-templates select="$ResultatTroisiemePasse/descendant::tei:div[@type='chapitre'][50]" mode="quatriemePasse"
